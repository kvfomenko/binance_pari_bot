CREATE OR REPLACE FUNCTION bina.f_get_chart(av_telegram_id IN VARCHAR,
...
<script>
    ...
    labels: [{{labels}}],
    datasets: [
{type: "{{price_data_type}}", label: "{{symbol}} price", hidden: false, borderWidth: 1, backgroundColor: "rgb(150, 150, 150)", borderColor: "rgb(150, 150, 150)", tension: 0.3, data: [{{prices}}]},
    ...
    const footer = (tooltipItems) => {
        ...
        tooltipItems.forEach(function(tooltipItem) {
            if (tooltipItem.datasetIndex == 0) {
                ...
                result = "Deviation:";
                for (let i=0+1;i<=datasets-2;i++) {
                    ema[i] = data.datasets[i].data[tooltipItem.dataIndex];
                    dev[i] = trimToDecimalPlaces(price[0] / ema[i] * 100 - 100, 2) + '' .. '' + trimToDecimalPlaces(price[1] / ema[i] * 100 - 100, 2);
  ...
  new Chart(ctx, {
    ...
    IF lv_scale = '10s' THEN
        ...
        INTO lv_labels, lv_prices,
            lv_ema2, lv_ema3, lv_ema4, lv_ema5, lv_ema6
        FROM (SELECT
                  r.symbol,
    ...
    ELSIF lv_scale IN ('1s'/*, '1m'*/) THEN
        SELECT
            ...
            STRING_AGG(x.ema[5]::VARCHAR, ',' ORDER BY x.price_date),
            STRING_AGG(x.ema[6]::VARCHAR, ',' ORDER BY x.price_date)
        INTO lv_labels, lv_prices, lv_ema2, lv_ema3, lv_ema4, lv_ema5, lv_ema6
        FROM (SELECT
                  r.symbol,
                  r.price_date,
                  ROUND(CASE
                            ...
                            ELSE r.price
                        END::NUMERIC, 4) AS open_price,
                  ROUND(r.price::NUMERIC, 4) AS close_price,
                  ROUND(r.min_price::NUMERIC, 4) AS min_price,
                  ROUND(r.max_price::NUMERIC, 4) AS max_price,
